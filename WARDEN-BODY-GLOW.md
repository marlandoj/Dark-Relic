...
Run `integration/run_body_glow_build.ps1` from that delivery. It copies the baseline into `H:\DarkRelicBodyGlow-20260913`, compiles the editor plugin, creates the skeletal-mesh-compatible material, binds it in the realistic map, runs gameplay checks and captures charge, peak, fade and off states. The driver preserves existing releases and refuses to reuse an existing receipt without inspection.
