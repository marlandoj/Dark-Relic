param([string]$Delivery='H:\DarkRelicEnemyFeedbackDelivery-20260912')
$ErrorActionPreference='Stop'
$Root='H:\DarkRelicEnemyFeedback-20260912'
$Source='H:\DarkRelicAura-20260912'
...
Save-State
try {
    if ((Test-Path $Root) -or (Test-Path 'H:\DarkRelicEnemyFeedbackPackage')) { throw 'Candidate destination already exists' }
    ...
    $Prior=Get-Content "$Source\IntegrationEvidence\aura-build.json" -Raw | ConvertFrom-Json
    ...
    $Protected=@()
    foreach($Package in @('H:\DarkRelicPlayablePackage','H:\DarkRelicCharacterPackage','H:\DarkRelicEnhancedPackage','H:\DarkRelicWardenPackage','H:\DarkRelicFeedbackPackage','H:\DarkRelicAuraPackage')) {
    ...
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$Root\IntegrationEvidence\run_enemy_feedback_build.ps1" -Package *> "$Delivery\build-launch.log"
    $State.buildExit=$LASTEXITCODE
    if ($LASTEXITCODE -ne 0) { throw 'Enemy feedback build failed; read enemy-build.json' }
    $State.phase='passed'
