...
try:
    ...
    fresnel = node(unreal.MaterialExpressionFresnel, exponent=3.0, base_reflect_fraction=0.025)
    crimson = node(unreal.MaterialExpressionConstant3Vector, constant=unreal.LinearColor(3.2,0.035,0.008,1))
    gold = node(unreal.MaterialExpressionConstant3Vector, constant=unreal.LinearColor(5.0,1.25,0.08,1))
    ...
    opacity = node(unreal.MaterialExpressionMultiply, const_b=0.3)
    connect(fresnel,opacity,'A')
