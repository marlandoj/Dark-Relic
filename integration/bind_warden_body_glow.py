...
try:
    ...
    crimson = node(unreal.MaterialExpressionConstant3Vector, constant=unreal.LinearColor(1.6,0.025,0.012,1))
    gold = node(unreal.MaterialExpressionConstant3Vector, constant=unreal.LinearColor(4.0,0.75,0.065,1))
    ...
    rim = node(unreal.MaterialExpressionMultiply, const_b=0.55)
    connect(fresnel,rim,'A')
    opacity = node(unreal.MaterialExpressionAdd, const_b=0.22)
