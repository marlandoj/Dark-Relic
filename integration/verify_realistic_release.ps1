...
$Package='H:\DarkRelicRealisticPackage\Windows\DarkRelicSmoke'
...
$Receipt="$Evidence\realistic-release.json"
...
Save-State
try {
    $Build=Get-Content "$Evidence\realistic-package.json" -Raw | ConvertFrom-Json
    if(!$Build.complete -or $Build.phase -ne 'passed') { throw 'Realistic package is not verified' }
    ...
    if(Test-Path $Shortcut) {
        if($Link.TargetPath -ne 'H:\DarkRelicRealisticPackage\Windows\DarkRelicSmoke.exe' -or $Link.Arguments -ne '-dx11 -windowed -ResX=1920 -ResY=1080') { throw 'Existing candidate shortcut differs; preserve it for inspection' }
    }
    $Link.TargetPath='H:\DarkRelicRealisticPackage\Windows\DarkRelicSmoke.exe'
