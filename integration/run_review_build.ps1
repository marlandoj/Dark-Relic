...
$Receipt="$Evidence\polish-build.json"
...
try {
    ...
    $State.phase='map-polish'; Save-State
    & "$Engine\Binaries\Win64\UnrealEditor-Cmd.exe" "$Root\DarkRelicSmoke.uproject" -run=pythonscript "-script=$Evidence/polish_review.py" -unattended -NullRHI "-abslog=$Evidence\review-polish.log"
    $State.polishExit=$LASTEXITCODE; Save-State
    $Polish=Get-Content "$Evidence\review-polish.json" -Raw | ConvertFrom-Json
    if($LASTEXITCODE -ne 0 -or !$Polish.passed) { throw 'Map polish failed' }
    $State.phase='bindings'; Save-State
    & "$Engine\Binaries\Win64\UnrealEditor-Cmd.exe" "$Root\DarkRelicSmoke.uproject" -run=pythonscript "-script=$Evidence/bind_enemy_feedback.py" -unattended -NullRHI "-abslog=$Evidence\enemy-bindings.log"
    $State.bindingsExit=$LASTEXITCODE; Save-State
    $Bindings=Get-Content "$Evidence\enemy-bindings.json" -Raw | ConvertFrom-Json
    ...
    if(!$Runtime.passed -or $Runtime.checks -lt 89) { throw 'Character runtime checks did not pass' }
    ...
    if(!(Test-Path "$Evidence\playable-frame.png")) { throw 'Capture file missing' }
        ...
        if(!$Bindings.passed -or !$Runtime.passed -or $Runtime.checks -lt 89) { throw 'Cannot resume without passing character bindings and runtime evidence' }
    ...
    if($Package) {
        ...
        & "$Engine\Build\BatchFiles\RunUAT.bat" BuildCookRun "-project=$Root\DarkRelicSmoke.uproject" -nop4 -platform=Win64 -clientconfig=Development -nocompileeditor -skipbuildeditor -build -cook "-map=$Map" -stage -pak -iostore -archive -archivedirectory=H:/DarkRelicPolishPackage -utf8output *> "$Evidence\character-package.log"
        $State.packageExit=$LASTEXITCODE; Save-State
        if($LASTEXITCODE -ne 0) { throw 'Character packaging failed' }
        $PackageEvidence='H:\DarkRelicPolishPackage\Windows\DarkRelicSmoke\IntegrationEvidence'
        New-Item -ItemType Directory $PackageEvidence -Force | Out-Null
        Run-Game 'H:\DarkRelicPolishPackage\Windows\DarkRelicSmoke.exe' @('-DarkRelicSmoke','-unattended','-dx11','-windowed','-ResX=1920','-ResY=1080',"-abslog=$Evidence\character-packaged-runtime.log") 'packaged-runtime'
        $Packaged=Get-Content "$PackageEvidence\runtime-smoke.json" -Raw | ConvertFrom-Json
        if(!$Packaged.passed -or $Packaged.checks -lt 89) { throw 'Packaged character smoke failed' }
        Copy-Item "$PackageEvidence\runtime-smoke.json" "$Evidence\character-packaged-runtime.json"
        Run-Game 'H:\DarkRelicPolishPackage\Windows\DarkRelicSmoke.exe' @('-DarkRelicCapture','-unattended','-dx11','-windowed','-ResX=1920','-ResY=1080',"-abslog=$Evidence\character-packaged-capture.log") 'packaged-capture'
