...
$Package='H:\DarkRelicEnemyFeedbackPackage\Windows\DarkRelicSmoke'
$Delivery='H:\DarkRelicEnemyFeedbackDelivery-20260912'
$State=@{complete=$false;passed=$false;phase='preflight';started=[DateTime]::UtcNow.ToString('o');pid=$PID}
$Receipt="$Evidence\enemy-release.json"
function Save-State { $State | ConvertTo-Json -Depth 6 | Set-Content $Receipt }
Save-State
try {
    $Build=Get-Content "$Evidence\enemy-build.json" -Raw | ConvertFrom-Json
    ...
    $State.packageHashes=@(Get-ChildItem 'H:\DarkRelicEnemyFeedbackPackage' -Recurse -File | Where-Object {$_.Extension -in '.exe','.pak','.ucas','.utoc'} | ForEach-Object {@{path=$_.FullName;sha256=(Get-FileHash $_.FullName -Algorithm SHA256).Hash}})
    $Shortcut=Join-Path ([Environment]::GetFolderPath('Desktop')) 'Dark Relic Enemy Feedback.lnk'
    if(Test-Path $Shortcut) { throw 'Candidate shortcut already exists; inspect before replacement' }
    $Link=$Shell.CreateShortcut($Shortcut)
    $Link.TargetPath='H:\DarkRelicEnemyFeedbackPackage\Windows\DarkRelicSmoke.exe'
    $Link.Arguments='-dx11 -windowed -ResX=1920 -ResY=1080'
    $Link.WorkingDirectory='H:\DarkRelicEnemyFeedbackPackage\Windows'
    $Link.Save()
