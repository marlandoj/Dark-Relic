...
$Package='H:\DarkRelicAuraPackage\Windows\DarkRelicSmoke'
$Delivery='H:\DarkRelicAuraDelivery-20260912'
$State=@{complete=$false;passed=$false;phase='preflight';started=[DateTime]::UtcNow.ToString('o');pid=$PID}
$Receipt="$Evidence\aura-release.json"
function Save-State { $State | ConvertTo-Json -Depth 6 | Set-Content $Receipt }
Save-State
try {
    $Build=Get-Content "$Evidence\aura-build.json" -Raw | ConvertFrom-Json
    ...
    $State.packageHashes=@(Get-ChildItem 'H:\DarkRelicAuraPackage' -Recurse -File | Where-Object {$_.Extension -in '.exe','.pak','.ucas','.utoc'} | ForEach-Object {@{path=$_.FullName;sha256=(Get-FileHash $_.FullName -Algorithm SHA256).Hash}})
    $Shortcut=Join-Path ([Environment]::GetFolderPath('Desktop')) 'Dark Relic Fury Aura.lnk'
    if(Test-Path $Shortcut) { throw 'Candidate shortcut already exists; inspect before replacement' }
    $Link=$Shell.CreateShortcut($Shortcut)
    $Link.TargetPath='H:\DarkRelicAuraPackage\Windows\DarkRelicSmoke.exe'
    $Link.Arguments='-dx11 -windowed -ResX=1920 -ResY=1080'
    $Link.WorkingDirectory='H:\DarkRelicAuraPackage\Windows'
    $Link.Save()
