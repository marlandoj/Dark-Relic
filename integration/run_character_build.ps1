...
if(Test-Path $Receipt) {
    ...
    Copy-Item $Receipt "$Evidence\character-build-$([DateTime]::UtcNow.ToString('yyyyMMddTHHmmss')).json"
}
...
try {
    ...
    if(!$Runtime.passed -or $Runtime.checks -lt 27) { throw 'Character runtime checks did not pass' }
    ...
    if(!(Test-Path "$Evidence\playable-frame.png")) { throw 'Capture file missing' }
        ...
        if(!$Bindings.passed -or !$Runtime.passed -or $Runtime.checks -lt 27) { throw 'Cannot resume without passing character bindings and runtime evidence' }
    ...
    if($Package) {
        ...
        $Config=$Config -replace '(?m)^GameDefaultMap=.*$',"GameDefaultMap=$Map.LVL_DarkRelicCharacters"
        ...
        & "$Engine\Build\BatchFiles\RunUAT.bat" BuildCookRun "-project=$Root\DarkRelicSmoke.uproject" -nop4 -platform=Win64 -clientconfig=Development -nocompileeditor -skipbuildeditor -build -cook "-map=$Map" -stage -pak -iostore -archive -archivedirectory=H:/DarkRelicCharacterPackage -utf8output *> "$Evidence\character-package.log"
        $State.packageExit=$LASTEXITCODE; Save-State
        if($LASTEXITCODE -ne 0) { throw 'Character packaging failed' }
        $PackageEvidence='H:\DarkRelicCharacterPackage\Windows\DarkRelicSmoke\IntegrationEvidence'
        New-Item -ItemType Directory $PackageEvidence -Force | Out-Null
        Run-Game 'H:\DarkRelicCharacterPackage\Windows\DarkRelicSmoke.exe' @('-DarkRelicSmoke','-unattended','-dx11','-windowed','-ResX=1920','-ResY=1080',"-abslog=$Evidence\character-packaged-runtime.log") 'packaged-runtime'
        $Packaged=Get-Content "$PackageEvidence\runtime-smoke.json" -Raw | ConvertFrom-Json
        if(!$Packaged.passed -or $Packaged.checks -lt 27) { throw 'Packaged character smoke failed' }
        Copy-Item "$PackageEvidence\runtime-smoke.json" "$Evidence\character-packaged-runtime.json"
        Run-Game 'H:\DarkRelicCharacterPackage\Windows\DarkRelicSmoke.exe' @('-DarkRelicCapture','-unattended','-dx11','-windowed','-ResX=1920','-ResY=1080',"-abslog=$Evidence\character-packaged-capture.log") 'packaged-capture'
